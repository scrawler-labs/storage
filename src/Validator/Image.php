...
class Image extends \Scrawler\Validator\AbstractValidator
{
    ...
    protected $utf7Signatures = array(
        '2b4146732d',
        '2b4148302d',
        '2b4148732d',
        '2b4144302d',
    ...
    protected $htmlSignatures = array(
    ...
    protected $exploitSignatures = array(
    ...
    private function scanSource($string)
    {
        ...
        if (is_array($string)) {
            ...
            if (preg_match("/(3c|c2bc|2575|5c75|253363|26233630|50413d3d|2b4144772d)([a-z-0-9]|5c3f|25|23|5c2b41434d2d|3c|21|5c24|5c2a|40)/mx", $string)) {
                ...
                for ($i = 0; $i < count($this->xssSignatures); $i++) {
                ...
                if ($sieve >= 1) {
                    ...
                    for ($i = 0; $i < count($this->exploitSignatures); $i++) {
                        if (stristr($string, $this->exploitSignatures[$i]) !== FALSE) {
    ...
    private function processImage(UploadedFile $file): UploadedFile
    {
        ...
        list($w, $h) = getimagesize($file->getClientOriginalName());
        ...
        switch ($this->mime) {
            case $normalize[0]:
                if (isset($gdcheck["GIF Create Support"]) == true) {
                    $ext = '.gif';
                    $image = imagecreatefromgif($file->getClientOriginalName());
                    $resampled = imagecreatetruecolor($new_width, $new_height);
                    imagecopyresampled($resampled, $image, 0, 0, 0, 0, $new_width, $new_height, $w, $h);
                    imagegif($resampled, $file->getClientOriginalName());
                    $endsize = filesize($file->getClientOriginalName());
                } else {
                    $gd = FALSE;
                }
                break;
            case $normalize[1]:
                if (isset($gdcheck["JPG Support"]) == true || isset($gdcheck["JPEG Support"]) == true) {
                    ...
                    $resampled = imagecreatetruecolor($new_width, $new_height);
                    imagecopyresampled($resampled, $image, 0, 0, 0, 0, $new_width, $new_height, $w, $h);
                    imagejpeg($resampled, $file->getClientOriginalName(), 100);
                    $endsize = filesize($file->getClientOriginalName());
                } else {
                    $gd = FALSE;
                }
            ...
            case $normalize[2]:
                if (isset($gdcheck["PNG Support"]) == true) {
                    $ext = '.png';
                    $resampled = imagecreatetruecolor($new_width, $new_height);
                    $image = imagecreatefrompng($file->getClientOriginalName());
                    imagealphablending($image, true);
                    imagesavealpha($image, true);
                    imagecopyresampled($resampled, $image, 0, 0, 0, 0, $new_width, $new_height, $w, $h);
                    imagepng($resampled, $file->getClientOriginalName(), 100);
                    $endsize = filesize($file->getClientOriginalName());
                } else {
                    $gd = FALSE;
                }
